...
var i18nIdHashing = function i18nIdHashing(_ref) {
   ...
   * `defaultMessages`
   */
  var getIdentifiersWithNamespacedIds = function getIdentifiersWithNamespacedIds(opts) {
  ...
  return {
    visitor: {
      CallExpression: function CallExpression(pathNode, state) {
        ...
        if (referencesImport(callee, moduleSourceName, FUNCTIONS_THAT_DEFINE_MESSAGES) === false) {
          return;
        }
      ...
      MemberExpression: function MemberExpression(pathNode, state) {
        ...
        if (!(getIdentifiersWithNamespacedIds(state.opts).indexOf(pathNode.node.object.name) !== -1)) {
          return;
        }
