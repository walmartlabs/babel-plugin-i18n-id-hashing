...
var i18nIdHashing = function i18nIdHashing(_ref) {
  ...
  var processMessage = function processMessage(filename, messageObj) {
    if (!(messageObj[1] && messageObj[1].isObjectExpression())) {
      throw _path2.default.buildCodeFrameError("[babel-plugin-i18n-id-hashing] " + messageObj.node.name + "() must be called with message " + "descriptors defined as object expressions.");
    }
